...
open OUT1, ">Introns_with_g4_chr$i.out" or die $!;#Modify to desired output file
open OUT2, ">Introns_without_g4_chr$i.out" or die $!;
...
open my $fh, '<', "Introns_Start_End_chr$i.out";#File with start co-ordinates viz the minimum positions
...
while (<$fh>) {
  ...
  for my $val (@g4) {
    last if $val > $max;
    push @introns_with_g4, $val if $val >= $min && $val <= $max;
  }
