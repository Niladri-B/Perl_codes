...
#Code to open 1000 files & compute dstrm values

#Open output file
open OUT, ">Dstrm_multiple_files2.out" or die $!;

#Open files
for(my $i = 1; $i <= 1; $i++){
	my $i = "Uniquely_Nearest_G4_LSD1_$i.out";
	print $i;
	...
	while(<$fh>){
		my @val = split;
		#say $val[3];
		if($val[3] eq "No_Match" or $val[3] eq "No" or $val[3] eq "Match"){#It is CRITICAl that No Match is the 1st Condition & we skipt to NEXT iteration of loop # Otherwise, it Perl treats literal string in numeric context as FALSE ie 0 and adds it in 0 to 250 bin
