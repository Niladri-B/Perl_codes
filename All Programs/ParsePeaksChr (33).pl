...
#
#THIS IS MODIFIED # THIS PRINTS OUT FIRST 3 ELEMENTS OF EACH ROW FROM SOURCE FILE
#pseudocode
...
my $file = 'K562_REST_peak_coordinates.txt';
...
while(<$fh>){
	...
	if($val[0] eq 'chr1'){
		#print "@val \n";
		 say OUT1 join ("\t",$val[0],$val[1],$val[2]); #Note: If you want to separate by tab, write "\t" at beginning followed by items you want to print
		
	}elsif($val[0] eq 'chr2'){
		say OUT2 join("\t",$val[0],,$val[1],$val[2]);#Note: If you separate the items you want by "\t", in addition to "\t" at beginning, you will put DOUBLE TAB
		
	}elsif($val[0] eq 'chr3'){
		say OUT3 join("\t",$val[0],$val[1],$val[2]);
		
	}elsif($val[0] eq 'chr4'){
		say OUT4 join("\t",$val[0],$val[1],$val[2]);
		
	}elsif($val[0] eq 'chr5'){
		say OUT5 join("\t",$val[0],$val[1],$val[2]);
	
	}elsif($val[0] eq 'chr6'){
		say OUT6 join("\t",$val[0],$val[1],$val[2]);
	
	}elsif($val[0] eq 'chr7'){
		say OUT7 join("\t",$val[0],$val[1],$val[2]);
	
	}elsif($val[0] eq 'chr8'){
		say OUT8 join("\t",$val[0],$val[1],$val[2]);
	
	}elsif($val[0] eq 'chr9'){
		say OUT9 join("\t",$val[0],$val[1],$val[2]);
	
	}elsif($val[0] eq 'chr10'){
		say OUT10 join("\t",$val[0],$val[1],$val[2]);
		
	}elsif($val[0] eq 'chr11'){
		say OUT11 join("\t",$val[0],$val[1],$val[2]);
	
	}elsif($val[0] eq 'chr12'){
		say OUT12 join("\t",$val[0],$val[1],$val[2]);
		
	}elsif($val[0] eq 'chr13'){
		say OUT13 join("\t",$val[0],$val[1],$val[2]);
	
	}elsif($val[0] eq 'chr14'){
		say OUT14 join("\t",$val[0],$val[1],$val[2]);
	
	}elsif($val[0] eq 'chr15'){
		say OUT15 join("\t",$val[0],$val[1],$val[2]);
	
	}elsif($val[0] eq 'chr16'){
		say OUT16 join("\t",$val[0],$val[1],$val[2]);
	
	}elsif($val[0] eq 'chr17'){
		say OUT17 join("\t",$val[0],$val[1],$val[2]);
	
	}elsif($val[0] eq 'chr18'){
		say OUT18 join("\t",$val[0],$val[1],$val[2]);
	
	}elsif($val[0] eq 'chr19'){
		say OUT19 join("\t",$val[0],$val[1],$val[2]);
	
	}elsif($val[0] eq 'chr20'){
		say OUT20 join("\t",$val[0],$val[1],$val[2]);
	
	}elsif($val[0] eq 'chr21'){
		say OUT21 join("\t",$val[0],$val[1],$val[2]);
	
	}elsif($val[0] eq 'chr22'){
		say OUT22 join("\t",$val[0],$val[1],$val[2]);
	
	}elsif($val[0] eq 'chrX'){
		say OUT23 join("\t",$val[0],$val[1],$val[2]);
	
	}else{
		say OUT24 join("\t",$val[0],$val[1],$val[2]);
	}
