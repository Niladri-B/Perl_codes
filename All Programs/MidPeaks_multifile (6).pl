...
for(my $j = 1; $j <= 1000; $j++){ #$j = COREST trial run	#$i = Chromosome Number
	...
	#Open Output file
	open OUT,">Mid_Random_REST_trial_$j=chr$i.out" or die $!;#Output file must be opened here otherwise $i becomes NOT JUST the NUMBER but ENTIRE FILENAME as specified below
		
	my $i = "Peaks_Random_REST_trial_$j.bed=chr$i.out";
	print $i;
