...
#Code to extract LSD1 genes given RPKM genes [THIS IS FOR BEDTOOLS INPUT]
  ...
  open OUT, ">RPKM_COREST_100p_g4_A.out" or die $!;

  my @file2;
  {
    open my $fh, '<', "COREST_A_100bp_g4_genes.out";#"Common_LSD1_COREST_REST_H3K4me1_H3K4me2_B_genes.out";#Modify to Midpg4chr*.out
    while (<$fh>) {
      chomp;
