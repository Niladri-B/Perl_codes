...
for(my $j = 1; $j <= 1000; $j++){
        ...
        for(my $i =1 ; $i <= 24; $i++){
        	#We want to run multiple loops together here for comparing COREST & G4 file together
        	#A Nested loop would not accomplish that as for each COREST it would look at all 24 G4 files and then move to 2nd COREST file
        	
        	open OUT, ">H3K4me1_g4_distance_trial_$j=chr$i.out" or die $!;
        	
                my $file1 = "SortedMid_Random_H3K4me1_trial_$j=chr$i.out.out";
		...
		#Assign filename
		#my $file1 = 'SortedMid_Random_COREST_trial_1=chr1.out';#'lsdpeakmid.txt'; #case insensitive
		...
		while (<$fh1>){
			chomp;
			my ($k, $v)=split(/\s+/); #split on whitespace
			my $bef=first{$_ >= $v}@{$h1{$k}}; #first($_) greater value from hash greater than COREST $v value
