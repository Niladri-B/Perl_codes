...
open OUT1, ">Promoters_with_Alu_chr$i.out" or die $!;#Modify to desired output file
open OUT2, ">Promoters_without_Alu_chr$i.out" or die $!;

my @g4;#@file2 => @g4
{
  open my $fh, '<', "MidVal_peak_Alu_chr$i.out" or die $!;#Modify to Midpg4chr*.out
  while (<$fh>) {
    chomp;
    push @g4, (split /\t/)[1];
  }
