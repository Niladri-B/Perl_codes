...
#for(my $j = 1; $j <= 2; $j++){#Loop for file number
        for(my $i =1 ; $i <= 23; $i++){#Loop for chromosome & G4 file
        	
        	#Open output file
        	#open OUT, ">LSD1_g4_distance_trial_$j=chr$i.out" or die $!;
        	open OUT, ">LSD1_nearest_g4_chr$i.out" or die $!;
        	
        	#Assign filenames
                #my $file1 = "SortedMid_Random_LSD1_trial_$j=chr$i.out.out";
                my $file1 = "MidVal_peak_signal_chr$i.out";
		...
		while (<$fh1>){
			chomp;
			my ($k, $v, $signal)=split(/\s+/); #split on whitespace
			...
			my $str=sprintf("%-8s %-10d %-5s %-8s %-8s %-8s",$k, $v,$bef?"No":"Yes",$bef?$bef:"0",$aft?$aft:"0", $signal); #$v = number, hence %d used, others have strings as input hence %s is used
